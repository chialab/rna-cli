...
/**
 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 * @TODO remove when https://github.com/rollup/rollup-plugin-commonjs/issues/303 has been resolved
 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 */
...
function defaultResolver(importee, importer) {
    ...
    if (isAbsolute(importee)) {
        return addJsExtensionIfNecessary(path.resolve(importee));
    }

    // if this is the entry point, resolve against cwd
    if (importer === undefined) {
        return addJsExtensionIfNecessary(path.resolve(process.cwd(), importee));
    }
...
function transformCommonjs(parse, code, id, isEntry, ignoreGlobal, ignoreRequire, customNamedExports, sourceMap, allowDynamicRequire, astCache) {
    ...
    function isRequireStatement(node) {
        ...
        if (node.arguments.length !== 1 || (node.arguments[0].type !== 'Literal' && (node.arguments[0].type !== 'TemplateLiteral' || node.arguments[0].expressions.length > 0))) {
            return;
    ...
    estreeWalker.walk(ast, {
        enter: function enter(node, parent) {
            ...
            if (node.type === 'AssignmentExpression') {
                ...
                if (scope.contains(flattened$1.name)) {
                    return;
                }

                var match = exportsPattern.exec(flattened$1.keypath);
                if (!match || flattened$1.keypath === 'exports') {
                    return;
                }
                ...
                if (flattened$1.keypath === 'module.exports' && node.right.type === 'ObjectExpression') {
                    return node.right.properties.forEach(function(prop) {
                        ...
                        if (name === rollupPluginutils.makeLegalIdentifier(name)) {
    ...
    var named = namedExportDeclarations
        .filter(function(x) {
            return x.name !== 'default' || !hasDefaultExport;
        })
        .map(function(x) {
            return x.str;
        });
...
function commonjs(options) {
    ...
    var ignoreRequire = typeof options.ignore === 'function' ?
        options.ignore :
        Array.isArray(options.ignore) ? function(id) {
            return ~options.ignore.indexOf(id);
        } :
        function() {
            return false;
        };
    ...
    function resolveId(importee, importer) {
        ...
        return resolveUsingOtherResolvers(importee, importer).then(function(resolved) {
            if (resolved) {
                return isProxyModule ? PREFIX + resolved : resolved;
            }
    ...
    return {
        ...
        options: function options(options$1) {
            ...
            var isExternal = function(id) {
                return options$1.external ?
                    Array.isArray(options$1.external) ? ~options$1.external.indexOf(id) :
                    options$1.external(id) :
                    false;
            };

            resolvers.unshift(function(id) {
                return isExternal(id) ? false : null;
            });
            ...
            entryModuleIdsPromise = Promise.all(
                entryModules.map(function(entry) {

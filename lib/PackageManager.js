...
class PackageManager {
     ...
     */
    async install() {
        return await exec(BIN, ['--cwd', this.cwd, 'install', '--ignore-scripts', '-W']);
    }
    ...
    async add(...packages) {
        ...
        return await exec(BIN, ['--cwd', this.cwd, 'add', ...packages, '--ignore-scripts', '-W']);
    }
    ...
    async dev(...packages) {
        ...
        return await exec(BIN, ['--cwd', this.cwd, 'add', ...packages, '--dev', '--ignore-scripts', '-W']);
    }
    ...
    async remove(...packages) {
        ...
        return await exec(BIN, ['--cwd', this.cwd, 'remove', ...packages, '--ignore-scripts', '-W']);
    }
     ...
     */
    async start() {
        return await exec(BIN, ['--cwd', this.cwd, 'start']);
    }
     ...
     */
    async test() {
        return await exec(BIN, ['--cwd', this.cwd, 'test']);
    }
     ...
     */
    async run(cmd, args) {
        return await exec(BIN, ['--cwd', this.cwd, 'run', cmd, ...args]);
    }
}

...
class Store extends Directory {
    ...
    constructor(name) {
        ...
        this.temporaryPath = new Directory(path.join(TMP_DIR, name));
        ...
        // remove temporary files on process exit, if not in CI env.
        if (!process.env.CI) {
     ...
     * @return {File} The file reference in the tmp directory.
     */
    tmpfile(file) {
     ...
     * @return {Directory} The directory reference in the tmp directory.
     */
    tmpdir(directory) {
        // rename the directory using a time hash.
        let ref = this.temporaryPath.directory(temporaryRename(directory));
        this.temporaryFiles.push(ref);
        return ref;
    }
}

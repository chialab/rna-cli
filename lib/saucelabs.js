...
function browserslistConfig(entry, data) {
    ...
    if (!vm && (name === 'Google Chrome' || name === 'Firefox')) {
        let maxVersion = 0;
        data.filter((vm) => vm.long_name === name).forEach((vm) => {
        ...
        } else if (maxVersion + 2 == version) {
            vm = data.find((vm) => vm.long_name === name && vm.short_version === 'alpha');
        }
    }
...
module.exports = {
    ...
    getTestName(project, testType) {
        ...
        console.log('process.env', process.env.TRAVIS_BRANCH);
        const branchName = process.env.TRAVIS_BRANCH || process.env.CI_COMMIT_REF_NAME;

...
class CLI {
    ...
    async exec(command, options = {}) {
        ...
        if (command.deprecated) {
            this.logger.error(`${command.name} is deprecated since ${command.deprecated}${command.deprecatedMessage ? ` - ${command.deprecatedMessage}` : ''}`);
        }
    ...
    async checkUpdate() {
        ...
        try {
            ...
            // if unstable version, notify the user
            if (!this.isStableVersion()) {
                this.logger.info('RNA CLI unstable version detected. Please upgrade to a stable version.');
                return;
            }

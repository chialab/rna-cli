...
class CLI {
    ...
    help() {
        ...
        commandNames.forEach((name) => {
            this.helpCommand(this.commands[name], 3);
            this.logger.newline();
        });
        this.logger.newline();
    }

    helpCommand(command, indent) {
        let indentOptions = command.name.length + 1;
        // Display command-specific help text.
        this.logger.log(`${''.padStart(indent, ' ')}${command.name.padEnd(indentOptions)} ${command.desc || ''} ${command.deprecated ? `Deprecated since ${command.deprecated}` : ''}`);
        let optionsHelp = defaultOptionsHelp(command);
        if (optionsHelp) {
            optionsHelp.split('\n')
                .forEach((optionHelp) => {
                    this.logger.log(''.padStart(indentOptions + indent, ' '), optionHelp);
                });
        }
    }

...
class CLI {
    ...
    async start(argv) {
        ...
        const wrongCommands = commands.filter((name) => name._ !== '_').some((command) => !(command._ in this.commands));
        if (wrongCommands) {
            this.help();
            return;
        }
        ...
        if (this.options['v'] || this.options['version']) {
            ...
            for (let i = 0; i < commands.length; i++) {
                ...
                // execute the command
                try {
                    const exitCode = await this.exec(this.commands[cmd._], cmd);
     ...
     * Run a command.
     *
     * @param {Command|string} command The command to be executed.

...
module.exports = class Watcher {
    ...
    watch(callback) {
        ...
        if (log !== false) {
            ...
            console.log(`${colors.bold('watching files...')} ${this.files.length < 5 ? colors.grey(`(${this.files.join(', ')})`) : ''}`);
        }
    ...
    hash(file) {
        return crypto.createHash('md5')
            .update(file.readFileSync(file, 'utf8'), 'utf8')
            .digest('hex');
    }

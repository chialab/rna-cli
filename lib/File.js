...
class TmpFile extends File {
    constructor(extension = '') {
        const dir = os.tmpdir();
        const name = path.join(dir, `${Date.now()}${extension}`);
        super(name);
        process.on('exit', this.unlink.bind(this));
        process.on('SIGINT', this.unlink.bind(this));
        process.on('SIGUSR1', this.unlink.bind(this));
        process.on('SIGUSR2', this.unlink.bind(this));
        process.on('uncaughtException', this.unlink.bind(this));
    }
}

const { promises: { realpath, writeFile, stat, unlink, readFile, readdir, mkdir, copyFile, rmdir }, constants } = require('fs');
...
class Entry {
    ...
    constructor(file) {
        if (file instanceof Entry) {
            return new this.constructor(file.path);
        }
        this.path = file;
    }
...
class File extends Entry {
     ...
     */
    async hash() {
        let buffer = await readFile(this.path);
...
class Directory extends Entry {
    ...
    watch(options, callback) {
        ...
        let { ignore: ignoreRules } = options || {
            ignore: /(^|[/\\])\../,
        };
        ...
        return chokidar.watch(this.path, {
            ...
            for (let i = 0; i < ignoreRules.length; i++) {
                let ignoreRule = ignoreRules[i];
                if (ignoreRule instanceof RegExp && ignoreRule.test(entry.path)) {
                    return;
                }
                if (typeof ignoreRule === 'function' && ignoreRule(entry)) {
                    return;
                }
            }
...
class Project extends Directory {
    ...
    async getParent() {
        ...
        while (paths.length) {
            ...
            let workspaces = await superProject.getWorkspaces();
            if (!workspaces) {
     ...
     */
    async getWorkspaceDependencies(project) {
        let wss = await this.getWorkspaces();
        if (!wss) {
            return null;
        }
        let { map } = Project.sort(wss);
     ...
     * @param {number} level Should return legacy targets for a given level.
     * 0 - [dev] llast browsers version
     * 1 - es6 support
     * 2 - es5 support
     * @return {Promise<Array<string>>}
     */
    async browserslist(level = 0) {
        ...
        if (parent) {
            ...
            return parent.browserslist(level);
        }
        ...
        if (level === 1) {
            return Targets.fromFeatures('module', 'async').toQuery();
        }

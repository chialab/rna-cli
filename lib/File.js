    ...
    async publish(version, git, npm) {
        ...
        if (workspaces) {
            await Promise.all(
                workspaces.map(async (project) => {
                    ...
                    try {
                        ...
                        throw new Error(`failed to fetch version list from https://registry.npmjs.org/${project.get('name')}`);
                    }

                    let versions = Object.keys(response.versions);
                    if (versions.hasOwnProperty(newVersion)) {
                        throw new Error(`cannot publish version ${newVersion} for ${project.get('name')} because has been already published`);
                    }
                })
            );
            ...
            try {
                ...
                throw new Error(`failed to fetch version list from https://registry.npmjs.org/${this.get('name')}`);
            }
            ...
            if (versions.includes(newVersion)) {
                throw new Error(`cannot publish version ${newVersion} for ${this.get('name')} because has been already published`);
            }

    ...
    watch(options, callback) {
        ...
        return fs.watch(this.path, {
            ...
            if (!entry.exists()) {
                ...
                if (map.get(entry.path) === hash) {
                    return;
                }
                // update the hash map.
                map.set(entry.path, hash);
            }
            if (options.ignore) {
                const rules = Array.isArray(options.ignore) ? options.ignore : [options.ignore];
                const shouldIgnore = rules.some((ignoreRule) => {
                    if (ignoreRule instanceof RegExp) {
                        return ignoreRule.test(entry.path);
                    }
                    if (typeof ignoreRule === 'function') {
                        return ignoreRule(entry.path);
                    }
                    return false;
                });
                if (shouldIgnore) {

const { realpathSync, statSync, existsSync, readdirSync, mkdirSync, writeFileSync, readFileSync, openSync, readSync, closeSync, copyFileSync, unlinkSync, rmdirSync, promises: { realpath }, constants } = require('fs');
...
class Entry {
    ...
    copy(to) {
        ...
        copyFileSync(this.path, to, constants.COPYFILE_FICLONE);
    }
}
...
class Project extends Directory {
    ...
    static getTSConfig(entry) {
        ...
        let tsConfigFile;
        while (entry) {
            tsConfigFile = entry.file('tsconfig.json');
    ...
    static sort(projects) {
        ...
        projects.forEach((ws1) => {
            ...
            const index = list.findIndex((ws2) => {
                const name2 = ws2.get('name');
                const deps = map[name2] || [];
    ...
    setupWorkspaces() {
        ...
        const { list, map } = Project.sort(wss);
     ...
     * @return {Project[]} A list of linked dependencies.
     */
    getLinkedDependencies() {
        ...
        return dependencies
            ...
            .filter((dir) => dir !== realpathSync(dir))
            .map((dir) => new Project(realpathSync(dir)));
    }

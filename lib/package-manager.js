...
let api = {
     ...
     */
    addToCli(...packages) {
        let json = path.join(paths.cli, 'package.json');
        let clone = path.join(paths.cli, 'package.original.json');
        fs.copyFileSync(json, clone);
        return exec(`${BIN} --cwd ${paths.cli} add ${packages.join(' ')} --no-lockfile -W`)
            .then((res) => {
                fs.copyFileSync(clone, json);
                fs.removeSync(clone);

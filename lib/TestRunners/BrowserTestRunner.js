...
class BrowserTestRunner extends TestRunner {
    async setup(options = {}) {
        ...
        if (options.browser) {
            ...
            let filteredBrowsers = ['chrome', 'firefox'];
            if (typeof options.browser === 'string') {
                filteredBrowsers = options.browser.split(/[\s-_,]{1,}/).filter((browserName) => browserName.toLowerCase() in browsers);
            }

            config.browsers = filteredBrowsers.map((name) => browsers[name]);
        }
    ...
    async build(files) {
        ...
        this.entryPoint.write(karmaCode);

...
class BrowserTestRunner extends TestRunner {
    async setup(options = {}) {
        ...
        let config = {
            ...
            customContextFile: options.customContextFile ? options.customContextFile : path.join(__dirname, 'static', 'context.html'),
    ...
    async run(files) {
        ...
        indexFile.write(`System.import('./base/${this.options.root.relative(testFile)}').then(function() { window.__karma__.loaded(); })`);
        await bundler.setup({
            code: inputCode,
            input: testFile,
        ...
        config.files.unshift(
            {
                pattern: path.resolve(__dirname, '../Bundlers/libs/system.js'),
                included: true,
                served: true,
                nocache: false,
                watched: false,

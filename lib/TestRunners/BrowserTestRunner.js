    ...
    async run(files) {
        ...
        return await new Promise((resolve) => {
            ...
            const server = new karma.Server(config, (exitCode) => {
                let coverage = coverageMap.toJSON();
                resolve({
                    exitCode,
                    failed,
                    coverage,
            ...
            server.on('coverage_complete', (browser, coverageReport) => {
                ...
                global.__coverage__ = coverageReport;
                const reportDir = this.project.directory('reports').directory('coverage').directory(browser.name);
                const nyc = new NYC({
                    'check-coverage': true,
                    'per-file': true,
                    'lines': 100,

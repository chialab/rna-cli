...
class BrowserTestRunner extends TestRunner {
    ...
    async run(files) {
        ...
        const karmaCode = generateKarmaCode(this.options.root.relative(testFile));
        indexFile.write(karmaCode);
        await bundler.setup({
            code: inputCode,
            input: testFile,
        ...
        this.addResources(...watchFiles
            ...
            .filter((filePath) => filePath !== testFile.path && filePath !== indexFile.path)
        );

...
class BrowserTestRunner extends TestRunner {
    ...
    async build(files) {
        ...
        let specsFile = this.testDir.file('__specs__.js');
        let karmaCode = await generateKarmaCode(this.testDir.relative(specsFile));
        ...
        bundler.on(ScriptBundler.WARN_EVENT, (warning) => {
            this.emit(BrowserTestRunner.PREPARE_WARN_EVENT, warning);
        });
        ...
        await this.addResources(
            ...watchFiles.filter((filePath) => filePath !== specsFile.path && filePath !== this.entryPoint.path)
        );
    ...
    async run() {
        ...
        await new Promise((resolve, reject) => {
            ...
            server.on('browser_log', (browser, log) => {
                let reporter = reportersMap[browser.name];
                reporter.log(`${browser.name}: ${log}`);
            });

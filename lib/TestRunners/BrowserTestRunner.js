...
${this.options.polyfill ? promisePolyfillCode : ''}${systemCode}System.import('./base/${this.options.root.relative(testFile)}').then(function() { karmaStart(); })`);
        await bundler.setup({
            code: inputCode,
            input: testFile,
        ...
        this.result = await new Promise((resolve) => {
            ...
            server.on('browser_complete', (browser, result) => {
                if (result.coverage) {
                    this.reportCoverage(result.coverage, browser.name);
                }
            });

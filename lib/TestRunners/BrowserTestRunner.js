...
const detectBrowsers = require('./lib/detect-browsers');
...
class BrowserTestRunner extends TestRunner {
    ...
    async setup(options = {}) {
        ...
        if (options.browser) {
            ...
            if (typeof options.browser === 'string') {
                let available = options.browser
                    .split(/[\s-_,]{1,}/)
                    .filter((browserName) => browserName.toLowerCase() in browsers);
                this.addEnvironments(...available);
                config.browsers = available.map((browserName) => browsers[browserName.toLowerCase()]);
            } else {
                let available = detectBrowsers()
                    .filter((browserName) => browserName.toLowerCase() in browsers);
                this.addEnvironments(...available);
    ...
    async run() {
        ...
        this.emit(BrowserTestRunner.RUN_START_EVENT);
        ...
        this.result = result;
        this.emit(BrowserTestRunner.RUN_END_EVENT, result);

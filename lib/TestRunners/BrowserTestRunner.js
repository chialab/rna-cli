...
class BrowserTestRunner extends TestRunner {
    async setup(options = {}) {
        ...
        let config = {
            ...
            // context if any
            customContextFile: options.context ? options.context : path.join(__dirname, 'static', 'context.html'),
    ...
    async run(files) {
        ...
        indexFile.write(`System.import('./base/${this.options.root.relative(testFile)}').then(function() { window.__karma__.start(); })`);
        await bundler.setup({
            code: inputCode,
            input: testFile,

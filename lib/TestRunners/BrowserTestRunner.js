    ...
    async run() {
        ...
        await new Promise((resolve, reject) => {
            ...
            server.on('browser_register', (browser) => {
                ...
                browser.emitter.on('spec_complete', (browser, result) => {
                    this.emit(BrowserTestRunner.RUN_PROGRESS_EVENT, reporter, result.suite.join(' > '));
                });
            });
            ...
            server.on('browser_complete', async (browser, result) => {
                if (result && result.coverage) {
                    ...
                    await reporter.mergeCoverage(result.coverage);
                    await mainReporter.mergeCoverage(reporter.getReport().coverage);

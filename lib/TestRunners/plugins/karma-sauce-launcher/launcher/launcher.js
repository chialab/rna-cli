...
function SaucelabsLauncher(args, /* config.sauceLabs */ config, /* SauceConnect */ sauceConnect, browserMap, logger, baseLauncherDecorator, captureTimeoutLauncherDecorator, retryLauncherDecorator) {
    ...
    const { startConnect, sauceConnectOptions, sauceApiProxy, seleniumHostUrl, seleniumCapabilities, browserName, username, accessKey } = processConfig.processConfig(config, args);
    ...
    this.on('start', async (pageUrl) => {
        ...
        try {
            ...
            if (browserName.includes('iphone')) {
                // wait until browser is ready
                let title = await driver.getTitle();
                while (!title.includes('welcome')) {
                    title = await driver.getTitle();
                }
            }

            if (browserName.includes('android')) {
                pageUrl = pageUrl.replace('localhost', '127.0.0.1.nip.io');
            }

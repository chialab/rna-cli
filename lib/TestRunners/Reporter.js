...
function printFailed(failed) {
    ...
    return failed.map((test) => {
        let error;
        if (test.error && test.error.multiple) {
            if (multipleTest !== test) {
                multipleTest = test;
                multipleErr = [test.error].concat(test.error.multiple);
            }
            error = multipleErr.shift();
        } else {
            error = test.error;
        }
        ...
        if (index === -1) {
            ...
            errorMessage = stack.slice(0, index);
...
${colors.red(errorMessage.replace(/^[ ]*/gm, printIndent(indent + 1))).trim()}`;

        if (stack.trim()) {
            output += `\n${colors.grey(stack.trim().replace(/^[ ]*/gm, printIndent(indent + 1)))}`;
        }
...
class Reporter {
    static formatReport(report) {
        ...
        if (!this.isEmptyCoverage(coverageMap)) {
            output += '\n';
            output += this.formatCoverage(coverageMap);
            output += '\n';
        }
    ...
    add(test) {
        return this.suite.addTest(test);
    }

    skip(test) {
        if (test instanceof Test) {
            test = this.suite.find(test.path) || this.suite.addTest(test);
        } else {
            test = this.suite.find(test) || this.suite.addTest(test);
        }
    ...
    merge(report) {
        ...
        report.failed.forEach((test) => this.fail(test, test.error));

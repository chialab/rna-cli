...
class CLI {
    ...
    start() {
        ...
        if (!this.isStableVersion(this.v)) {
            this.log(`${colors.bold(colors.cyan('RNA CLI unstable version detected. Please upgrade to a stable version.'))}`);
        } else {
            update = this.checkUpdate().catch(() => global.Promise.resolve()); // Update check.
        }
        ...
        return global.Promise.all([update, promise])
            .then(() => {
                // If latest command was 'upgrade' then don't check for latest version (again).
                if (commands.find((elem) => elem._ === 'upgrade')) {
                    return global.Promise.resolve();
                }
                return update
                    .then((remoteVersion) => {

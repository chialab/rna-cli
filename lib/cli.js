 ...
 * @returns {any}
 */
Command.prototype.exec = function(options, profiler) {
    // warn if deprecated
    if (this.deprecated) {
        this.scope.log(colors.red(`${this.name} is deprecated since ${this.deprecated}`));
    }
    return this.callback(this.scope, options, profiler);
};
 ...
 * @returns {this}
 */
Command.prototype.deprecate = function(version) {
...
CLI.prototype.exec = function(commandName, options = {}) {
    ...
    let res = command.exec(options, profiler);
        ...
        .then(() => {
            update.then((remoteVersion) => {
                if (remoteVersion && this.v !== remoteVersion) {
                    ...
                    this.log('$ yarn global add git+https://gitlab.com/chialab/rna-cli');
                    this.log('');
                    this.log(colors.grey(sep));
                    this.log('');
                }

...
class StyleBundler extends Bundler {
    ...
    async build(invalidate) {
        ...
        let { input, output, code, root, map } = this.options;

        this.emit(StyleBundler.BUNDLE_START_EVENT, input, code);
        this.emit(StyleBundler.BUILD_START_EVENT, input, code);
        ...
        try {
            ...
            let data = code || await input.read();
            let config = {
            ...
            let dependencies = this.result.messages
                .filter(({ type }) => type === 'dependency')
                .map(({ file }) => file)
                .filter((file) => fs.existsSync(file));
            await this.addResources(...dependencies);
            ...
            this.emit(StyleBundler.BUILD_END_EVENT, input, code);
            this.emit(StyleBundler.BUNDLE_END_EVENT, this.result);
    ...
    async write() {
        ...
        this.emit(StyleBundler.WRITE_START_EVENT);

        if (output &&
            await output.exists() &&
            await output.isFile() &&
            await output.mapFile.exists()) {
            await output.mapFile.unlink();
        }

        if (externalMapFile) {
            await output.mapFile.write(this.result.map.toString());
        }
        await output.write(content);
        this.emit(StyleBundler.WRITE_PROGRESS_EVENT, output);

        if (this.result.files) {
            this.result.files
                .map((fileName) => output.parent.file(fileName))
                .forEach((file) => {
                    this.emit(StyleBundler.WRITE_PROGRESS_EVENT, file);
                });
        }

        this.emit(StyleBundler.WRITE_END_EVENT);
        await super.write();
        return output;
    }
}

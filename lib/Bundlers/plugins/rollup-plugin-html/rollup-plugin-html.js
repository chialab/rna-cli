            ...
            async transform(source, id) {
                ...
                await bundler.setup({
                    ...
                    handleStyleLink: async (input) => {
                        ...
                        let assetId = this.emitAsset(input.localPath.replace(input.extname, '.css'), code);
                    ...
                    handleAssetLink: async (input) => {
                        let assetId = this.emitAsset(input.path);
                        return `import.meta.ROLLUP_ASSET_URL_${assetId}`;
                    },
                });
...
wrapper.innerHTML = '${html.replace(/\n/g, '\\n').replace(/import\.meta\.(ROLLUP_ASSET_URL_|ROLLUP_CHUNK_URL_)([\d\w]+)/g, '\' + import.meta.$1$2 + \'')}';

...
module.exports = function(options = {}) {
    ...
    return {
        ...
        async transform(code, id) {
            ...
            let esModule = ESM_KEYWORDS.test(code);
            let buildOptions = {
                target: 'es2019',
                sourcefile: id,
            ...
            if (pragma) {
                if (pragmaDefault) {
                    ...
                    buildOptions.jsxFactory = `__JSX__.${pragma}`;
                }
            }
            if (pragmaFrag) {
                if (pragmaDefault) {
                    ...
                    buildOptions.jsxFragment = `__JSX__.${pragmaFrag}`;
                }
            }
            if (jsxEntries.length && pragmaImport) {
                if (pragmaDefault) {
                    ...
                    code = `${code}\nimport * as __JSX__ from '${pragmaImport}';`;
                }
            }
                ...
                result = await babel.transformAsync(result.code, Object.assign({
                    ...
                    plugins: [
                        ...
                        (legacy || coverage) && [require('@babel/plugin-transform-runtime'), {
                            ...
                            helpers: true,
                            regenerator: true,
                            useESModules: esModule,
                        }],
                        !esModule && require('@babel/plugin-transform-modules-commonjs'),

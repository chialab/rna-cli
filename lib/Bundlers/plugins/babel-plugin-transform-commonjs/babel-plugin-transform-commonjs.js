    ...
    return {
        visitor: {
            Program(program, state) {
                let { cache = GLOBAL_CACHE } = state.opts;
                let body = program.get('body');
                let filename = program.hub.file.opts.filename;
                let globals = program.scope.globals;
                let importDeclarations = body.filter((entry) => entry.isImportDeclaration());
                let exportDeclarations = body.filter((entry) => entry.isExportDeclaration());
                let IS_WRAPPED = '__commonJS' in program.scope.bindings;
                let HAS_EXPORTS = ('module' in globals) || ('exports' in globals);
                let HAS_REQUIRE = 'require' in globals;
                let IS_UMD = ('define' in globals && 'window' in globals);

                if (
                    importDeclarations.length ||
                    exportDeclarations.length > (IS_WRAPPED ? 1 : 0) ||
                    ('import' in globals)
                ) {
                    return;
                }
                ...
                let moduleId = cache.get(filename);
                // global commonjs modules scope.
                let globalObject = program.scope.generateUidIdentifier('global');
                let localRegistry = program.scope.generateUidIdentifier('registry');
                let localExports = types.memberExpression(localRegistry, types.stringLiteral(moduleId), true);
                let globalRegistryDecl = types.variableDeclaration(
                    'var',
                    [
                ...
                let globalAssignment = types.expressionStatement(
                    types.assignmentExpression(
                        '=',
                        localExports,
                        types.logicalExpression(
                ...
                let assignments = [globalRegistryDecl, globalAssignment];
                let imports = [];
                ...
                if (!IS_UMD) {
                    program.traverse({
                        ...
                        CallExpression(path) {
                            ...
                            let requiredArg = path.get('arguments.0');
                            ...
                            // add to the list.
                            let moduleName = requiredArg.node.value;
                            ...
                            let id = program.scope.generateUidIdentifier(moduleName);
                            let local = program.scope.generateUidIdentifier(moduleName);
                ...
                if (HAS_EXPORTS) {
                    ...
                    let moduleIdentifier = program.scope.generateUidIdentifier('module');
                    let named = {};
                    ...
                    let wrap = types.expressionStatement(
                    ...
                    let globalVars = Object.keys(globals).filter((name) => name in named);

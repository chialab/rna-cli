const fs = require('fs-extra');
const path = require('path');
const postcss = require('postcss');

module.exports = postcss.plugin('postcss-copy-plugin', (opts = {}) => {
    let defaultHandler;
    if (opts.dest) {
        defaultHandler = (file, url, decl, importee, mapImportee, root, result) => {
            if (!file || !fs.existsSync(file)) {
                result.missingFiles = result.missingFiles || [];
                result.missingFiles.push(file);
                return;
            }
            let relative;
            if (opts.basePath && file.indexOf(opts.basePath) !== -1) {
                relative = path.relative(opts.basePath, file);
            } else {
                relative = path.relative(mapImportee || importee, file).replace(/^(\.\.\/)+/, './');
            }
            let relativeDistFile = path.join('assets', path.basename(relative));
            let distFile = path.join(opts.dest, relativeDistFile);
            if (distFile !== file) {
                result.files = result.files || [];
                result.files.push(distFile);
                fs.copySync(file, distFile);
            }
            return relativeDistFile;
        };
    }
    return async (root, result) => {
        const promises = [];
        root.walkDecls((decl) => {
            if (decl.value && decl.value.indexOf('url(') > -1) {
                const urls = decl.value.match(/url\(['"]?.*?['"]?\)/ig).map((entry) => entry.replace(/^url\(['"]?/i, '').replace(/['"]?\)$/i, ''));
                const importee = decl.source.input && decl.source.input.file;
                if (!importee && !opts.basePath) {
                    return;
                }
                const map = decl.source.input.map;
                promises.push(
                    ...urls.map(async (url) => {
                        let res;
                        let file;
                        let mapImportee;
                        if (url.indexOf('data:') === 0) {
                            return;
                        }
                        if (map) {
                            let position = map.consumer().originalPositionFor(decl.source.start);
                            if (position && position.source) {
                                let resolved = path.resolve(opts.dest, position.source);
                                if (fs.existsSync(resolved)) {
                                    mapImportee = resolved;
                                }
                            }
                        }
                        if (importee) {
                            file = path.resolve(path.dirname(importee), url.split('?')[0]);
                        }
                        if (opts.basePath && (!file || !fs.existsSync(file))) {
                            file = path.resolve(opts.basePath, url.split('?')[0]);
                        }
                        if (mapImportee && (!file || !fs.existsSync(file))) {
                            file = path.resolve(path.dirname(mapImportee), url.split('?')[0]);
                        }
                        if (opts.handleCSSAsset) {
                            res = opts.handleCSSAsset(file, url, decl, importee, mapImportee, opts, root, result);
                            if (res instanceof Promise) {
                                res = await res;
                            }
                        }
                        if (!res && defaultHandler) {
                            res = defaultHandler(file, url, decl, importee, mapImportee, root, result);
                            if (res instanceof Promise) {
                                res = await res;
                            }
                        }
                        return {
                            url,
                            replace: res,
                            decl,
                        };
                    })
                );
            }
        });

        const replacements = await Promise.all(promises);
        replacements.forEach((info) => {
            let { decl, url, replace } = (info || {});
            if (replace) {
                decl.value = decl.value.replace(url, replace);
            }
        });
    };
});

...
module.exports = postcss.plugin('postcss-copy-plugin', (opts = {}) => {
    let defaultHandler;
    if (opts.dest) {
        defaultHandler = (file, url, decl, importee, mapImportee) => {
                ...
                promises.push(
                    ...urls.map(async (url) => {
                        ...
                        if (opts.handleCSSAsset) {
                            res = opts.handleCSSAsset(file, url, decl, importee, mapImportee, opts);
                            if (res instanceof Promise) {
                                res = await res;
                            }
                        }
                        if (!res && defaultHandler) {
                            res = defaultHandler(file, url, decl, importee, mapImportee);
                            if (res instanceof Promise) {
                                res = await res;
                            }
                        }

...
class HTMLBundler extends Bundler {
    ...
    async build(...invalidate) {
        ...
        try {
            ...
            if (invalidating) {
                this.emit(HTMLBundler.BUILD_START, input, null, invalidate);
            ...
            if (invalidating) {
                this.emit(HTMLBundler.BUILD_END);
            } else {
    ...
    async handleLinks(document) {
        ...
        let elements = [...document.querySelectorAll('link[href], a[href], iframe[href]')]
            ...
            .filter((element) => root.file(element.getAttribute('href')).exists());
    ...
    async handleAssetLink(input, root, element) {
        ...
        this.linksBundlers.push(bundler);
        this.addResources(...bundler.files);
        return outputRoot.relative(output);
    }
    ...
    async handleSources(document) {
        ...
        await Promise.all(
            elements.map(async (element) => {
                ...
                let input = root.file(href);
                let output = outputRoot.file(href);
                let bundler = new CopyBundler();
                this.listenBundler(bundler);
                await bundler.setup({
                    input,
                ...
                await bundler.build();
                element.src = outputRoot.relative(output);
                this.addResources(...bundler.files);
    ...
    async handleScripts(document) {
        ...
        if (scriptModules.length) {
            ...
            let code = withSrc.map((script) => `import './${root.relative(root.file(script.getAttribute('src')))}'`).join('\n');
    ...
    async handleScriptModules(code, root, elements, document) {
        ...
        primary.textContent = `import('./${outputRoot.relative(esmOutput.file(scriptInput.name))}');window.__esm__ = true;`;
        primary.setAttribute('type', 'module');
        this.addResources(...esmBundler.files);

        let promiseUrl = require.resolve('promise-polyfill/dist/polyfill.min.js', {
            paths: [__dirname],
        });
        ...
        systemOutput.write(`${readFileSync(promiseUrl, 'utf8')}\n${readFileSync(systemUrl, 'utf8')}`);
        ...
        legacy.textContent = `!window.__esm__ && System.import('./${outputRoot.relative(umdOutput.file(scriptInput.name))}')`;
    ...
    async write() {
        ...
        let bundlers = (this.linksBundlers || [])
            .concat(this.stylesBundlers || [])
            .concat(this.scriptsBundlers || [])
            .concat(this.sourcesBundlers || [])
            .concat(this.iconBundler ? [this.iconBundler] : [])

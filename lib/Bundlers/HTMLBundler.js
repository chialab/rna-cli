...
class HTMLBundler extends Bundler {
    ...
    async build(...invalidate) {
        ...
        try {
            ...
            let swBundle = await this.handleServiceWorker(document);
            ...
            if (swBundle) {
                this.setServiceWorker(document);
            }
    ...
    /**
     * Remove service worker script if empty.
     * @private
     * @param {Document} document The document to update.
     * @return {void}
     */
    setServiceWorker(document) {
        let script = document.querySelector('script[data-serviceworker]');
        if (!script) {
            return;
        }

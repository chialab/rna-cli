...
class HTMLBundler extends Bundler {
    ...
    async handleScriptModules(code, root, elements, document) {
        ...
        if (!format || ['es', 'esm'].includes(format)) {
            ...
            if (!supportModules) {
                moduleScript.textContent = `import('${this.base || '/'}${outputRoot.relative(esmOutput.file(scriptInput.name))}');window.__esm__ = true;`;
            } else {
            ...
            if (!supportModules) {
                ...
                nomoduleScript.textContent = `!window.__esm__ && System.import('${this.base || '/'}${outputRoot.relative(umdOutput.file(scriptInput.name))}')`;

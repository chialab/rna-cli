...
class HTMLBundler extends Bundler {
    ...
    async build(...invalidate) {
        ...
        try {
            ...
            if (invalidating) {
                ...
                if (this.options.get('base')) {
                    this.base = this.options.get('base');
    ...
    async handleLinks(document) {
        ...
        return await Promise.all(
            elements.map(async (element) => {
                ...
                const inputFile = this.options.get('input').parent.file(href);
                if (
                    type === 'text/css' ||
                    rel === 'stylesheet' ||
    ...
    async handleStyleLink(input, root, element) {
        ...
        await bundler.setup({
            ...
            handleAssets: this.options.get('handleCSSAssets'),
        });
        await bundler.build();
    ...
    async handleAssetLink(input, root, element) {
        ...
        this.linksBundlers.push(bundler);
        this.addResources(...bundler.files);
        return `${this.base || ''}${outputRoot.relative(output)}`;
    }
    ...
    async handleScriptModules(code, root, elements, document) {
        ...
        if (!format || ['es', 'esm'].includes(format)) {
            ...
            if (!supportModules) {
                moduleScript.textContent = `('noModule' in document.createElement('script')) && import('${this.base || '/'}${outputRoot.relative(esmOutput.file(scriptInput.name))}');window.__esm__ = true;`;
            } else {
            ...
            if (!supportModules) {
                ...
                const systemUrl = require('resolve').sync('systemjs/dist/s.min.js', {
                    basedir: __dirname,
                });
                const systemCode = readFileSync(systemUrl, 'utf8').replace(/\b(let|const)\b/g, 'var');
                const promisePolyfillUrl = require('resolve').sync('promise-polyfill/dist/polyfill.min.js', {
                    basedir: __dirname,
                });
                ...
                nomoduleScript.textContent = `!window.__esm__ && (function(){${promisePolyfillCode}${systemCode}System.import('${this.base || '/'}${outputRoot.relative(umdOutput.file(scriptInput.name))}');})();`;
    ...
    async handleWebManifest(document, manifestFile, iconFile) {
        ...
        await bundler.setup({
            ...
            name: this.options.get('title') || document.title,
            description: this.options.get('description') || (descriptionElement && descriptionElement.content) || '',
            theme: themeElement ? themeElement.content : '',
            icon: iconSource,
            lang: document.documentElement.lang,
            overrides: {
                scope: this.options.get('base'),
            },
        });
        await bundler.build();
        this.addResources(...bundler.files);
    ...
    async write() {
        ...
        if (this.options.get('serviceWorker') !== false && this.webmanifestBundler) {
            ...
            if (this.bundlerSW) {
                ...
                const inputSW = this.options.get('input').parent.file(manifest.serviceworker.src);
                const outputSW = this.options.get('output').parent.file(manifest.serviceworker.src);
                const bundlerSW = this.bundlerSW = new ServiceWorkerBundler();

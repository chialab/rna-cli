    ...
    async handleScriptModules(code, root, elements, document) {
        ...
        if (!format || format === 'esm') {
            ...
            if (!supportModules) {
                ...
                moduleScript.setAttribute('src', `${this.base.replace(/^\/*$/, '')}/${outputRoot.relative(esmOutput.file(scriptInput.name))}`);
            }
        ...
        umdScript.setAttribute('src', `${this.base.replace(/^\/*$/, '')}/${scriptInput.name}`);

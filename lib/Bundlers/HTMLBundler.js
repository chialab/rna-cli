...
class HTMLBundler extends Bundler {
    ...
    async handleLinks(document) {
        ...
        const elements = [...document.querySelectorAll('link[href], a[download][href], iframe[href]')]
            ...
            .filter((element) => {
                ...
                if (!file.exists()) {
                    this.emit(HTMLBundler.WARN_EVENT, `referenced file '${file.path}' does not exists`, this);
                    return false;
                }
                return true;
            });
    ...
    async handleSources(document) {
        ...
        const output = this.options.get('output');
        const outputRoot = output.parent;
        console.log([...document.querySelectorAll('[src]:not(script)')])
    ...
    async handleWebManifest(document, manifestFile, iconFile) {
        ...
        await bundler.setup({
            ...
            scope: this.options.get('base') || (baseElement ? baseElement.getAttribute('href') : ''),

...
class HTMLBundler extends Bundler {
    ...
    async handleScriptModules(code, root, elements, document) {
        ...
        if (!format || ['es', 'esm'].includes(format)) {
            ...
            if (!supportModules) {
                moduleScript.textContent = `(async () => ('noModule' in document.createElement('script')) && await import('${this.base || '/'}${outputRoot.relative(esmOutput.file(scriptInput.name))}'))();window.__esm__ = true;`;
            } else {

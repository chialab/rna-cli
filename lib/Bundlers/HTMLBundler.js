        ...
        try {
            ...
            if (invalidating) {
                ...
                if (this.options.get('styles') !== false) {
                    this.styleBundlers = await this.handleLinks(document);
                }
                if (this.options.get('scripts') !== false) {
                    this.scriptBundlers = await this.handleSources(document);
                }
    ...
    async handleLinks(document) {
        ...
        return await Promise.all(
            elements.map(async (element) => {
                ...
                await bundler.build();
                element.href = this.options.get('output').directory.relative(output);
                this.addResources(...bundler.files);
                return bundler;
            })
        );
    ...
    async handleSources(document) {
        ...
        return await Promise.all(
            elements.map(async (element) => {
                ...
                await bundler.build();
                element.src = this.options.get('output').directory.relative(output);
                this.addResources(...bundler.files);
                return bundler;
            })
        );

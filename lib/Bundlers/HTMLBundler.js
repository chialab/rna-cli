    ...
    async handleScriptModules(code, root, elements, document) {
        ...
        if (!format || ['es', 'esm'].includes(format)) {
            ...
            if (!supportModules) {
                ...
                nomoduleScript.textContent = `!window.__esm__ && (function(){${this.options.get('polyfill') ? promisePolyfillCode : ''}${systemCode}System.import('${this.base || '/'}${outputRoot.relative(umdOutput.file(scriptInput.name))}');})();`;

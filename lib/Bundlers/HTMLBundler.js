...
class HTMLBundler extends Bundler {
    ...
    async handleScripts(document) {
        ...
        let bundlers = await Promise.all(
            scripts.map(async (element) => {
                if (element.src) {
                    ...
                    await bundler.build();
                    element.setAttribute('type', 'text/javascript');
                    element.src = outputRoot.relative(outputFile);
                    return bundler;
                }
        ...
        if (scriptModules.length) {
            ...
            let scriptInput = input.parent.file(input.basename.replace('.html', '.m.js'));
            ...
            primary.src = outputRoot.relative(esmOutput.file(scriptInput.basename));
            primary.setAttribute('type', 'module');
            document.body.appendChild(primary);
            this.addResources(...esmBundler.files);

            let umdBundler = new ScriptBundler(this.app, this.project);
            ...
            legacy.textContent = `System.import('./${outputRoot.relative(umdOutput.file(scriptInput.basename))}')`;
            legacy.setAttribute('type', 'text/javascript');
            legacy.setAttribute('nomodule', '');
            document.body.appendChild(legacy);

            bundlers.push(esmBundler, umdBundler);
        }

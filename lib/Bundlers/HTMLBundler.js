    ...
    async handleScriptModules(code, root, elements, document) {
        ...
        if (!format || ['es', 'esm'].includes(format)) {
            ...
            if (!supportModules) {
                ...
                const systemContent = readFileSync(systemUrl, 'utf8').replace(/\b(let|const)\b/g, 'var');
                ...
                nomoduleScript.textContent = `!window.__esm__ && (function(){${systemContent}System.import('${this.base || '/'}${outputRoot.relative(umdOutput.file(scriptInput.name))}');})();`;

...
class HTMLBundler extends Bundler {
    ...
    async handleScriptModules(code, root, elements, document) {
        ...
        let targets = Targets.parse(this.options.get('targets'));
        let supportTargets = Targets.parse(babelData['es6.module']);
        ...
        if (!supportModules) {
            moduleScript.textContent = `import('./${outputRoot.relative(esmOutput.file(scriptInput.name))}');window.__esm__ = true;`;
        } else {
        ...
        if (!supportModules) {
            ...
            nomoduleScript.textContent = `!window.__esm__ && System.import('./${outputRoot.relative(umdOutput.file(scriptInput.name))}')`;

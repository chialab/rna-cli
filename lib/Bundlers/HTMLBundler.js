...
class HTMLBundler extends Bundler {
    ...
    async build(...invalidate) {
        ...
        try {
            ...
            this.result = {
                document: isFragment ? JSDOM.fragment(document.head.innerHTML + document.body.innerHTML) : document.documentElement,
                isFragment,
    ...
    async handleStyleCode(code, root) {
        const bundler = new StyleBundler(this.app, this.project);
        await bundler.setup({
            code,
            root,
    ...
    async write() {
        ...
        if (this.options.get('serviceWorker') !== false && this.webmanifestBundler) {
            ...
            if (manifest.serviceworker && manifest.serviceworker.src) {
                ...
                let bundlerSW = new ServiceWorkerBundler(this.app, this.project);

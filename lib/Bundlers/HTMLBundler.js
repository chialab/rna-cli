...
class HTMLBundler extends Bundler {
    ...
    async handleScripts(document) {
        ...
        if (scriptModules.length) {
            ...
            let code = withSrc.map((script) => `export * from './${root.relative(root.file(script.getAttribute('src')))}'`).join('\n');
    ...
    async handleScriptModules(code, root, elements, document) {
        ...
        if (!supportModules) {
            ...
            let systemUrl = join(__dirname, 'libs', 'system.js');
            let systemOutput = umdOutput.file('system.js');
    ...
    async handleWebManifest(document, manifestFile, iconFile) {
        ...
        if (!manifestOutput) {
            if (manifestLink) {
                manifestOutput = output.parent.file(manifestLink.getAttribute('href'));
            } else {

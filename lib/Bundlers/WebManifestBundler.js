...
class WebManifestBundler extends Bundler {
    ...
    async build() {
        ...
        if (icon && !manifest.icons) {
            ...
            this.iconGenerator = new IconBundler(this.app, this.project);
            await this.iconGenerator.setup({
                input: icon,
            ...
            let generated = await this.iconGenerator.build();
        ...
        if (manifest.serviceworker && manifest.serviceworker.src) {
            // let swPath = resolve(dirname(input), manifest.serviceworker.src);
            // this.serviceWorkerGenerator = new ServiceWorkerBundler({
            //     input: swPath,
            //     output:
            // });
        }

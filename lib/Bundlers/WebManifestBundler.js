...
class WebManifestBundler extends Bundler {
    ...
    async build(invalidate) {
        ...
        let { input, output, name, description, scope, theme, icon, lang, icons = this.constructor.icons, overrides = {}} = this.options;

        this.emit(WebManifestBundler.BUNDLE_START_EVENT, input);
        await this.addResources(input.path);

        try {
            this.emit(WebManifestBundler.BUILD_START_EVENT, input, null);

            let manifest;
            if (await input.isNew()) {
                manifest = {};
            } else {
                manifest = await input.readJson();
            }
            ...
            if (icon && !manifest.icons) {
                await this.addResources(icon.path);
                ...
                await this.iconBundler.setup({
                    ...
                    output: icons.map(({ name, size, width, height, gutter, background, type }) => {
                        ...
                        let iconOutput = output.parent.file(iconRelative).rename(name);
                        return {
                            file: iconOutput,
                            name,
                            size,
                ...
                manifest.icons = icons.map(({ size, name }) => {
                    ...
                    return {
                        src: output.parent.relative(icon.file),
                        sizes: `${size}x${size}`,
                        type: 'image/png',
                    };
                });
            ...
            this.emit(WebManifestBundler.BUILD_END_EVENT, input, null);
            this.emit(WebManifestBundler.BUNDLE_END_EVENT, this.result);
    ...
    async write() {
        ...
        this.emit(WebManifestBundler.WRITE_START_EVENT);
        if (this.iconBundler) {
            await this.iconBundler.write();
        }
        await output.writeJson(this.result);
        this.emit(WebManifestBundler.WRITE_PROGRESS_EVENT, output);
        this.emit(WebManifestBundler.WRITE_END_EVENT);
        await super.write();
        return output;
    }
}

...
class WebManifestBundler extends Bundler {
    ...
    async build(...invalidate) {
        ...
        const name = this.options.get('name');
        const description = this.options.get('description');
        const scope = this.options.get('scope');
        const theme = this.options.get('theme');
        const icon = this.options.get('icon');
        const lang = this.options.get('lang');
        const icons = this.options.get('icons') || this.constructor.icons;
        const overrides = this.options.get('overrides') || {};

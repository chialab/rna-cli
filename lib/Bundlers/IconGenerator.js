...
class IconGenerator extends EventEmitter {
    ...
    async build() {
        ...
        this.result = await Promise.all(
            output.map(async (iconDescriptor) => {
                let sourceBuffer = (await Jimp.read(input));
                if (iconDescriptor.type === 'icon') {
                    let { file, size, gutter, background } = iconDescriptor;
                    let iconBuffer = new Jimp(size, size, colorToString(background || { r: 255, g: 255, b: 255, alpha: 1 }));
                ...
                } else if (iconDescriptor.type === 'splash') {
                    let { file, width, height, gutter, background } = iconDescriptor;
                ...
                throw `invalid icon type: ${iconDescriptor.type}`;
            })
        );

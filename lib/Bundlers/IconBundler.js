...
class IconBundler extends Bundler {
    ...
    async build(...invalidate) {
        ...
        this.emit(IconBundler.BUNDLE_START_EVENT, input);
        this.emit(IconBundler.BUILD_START_EVENT, input);
        ...
        this.emit(IconBundler.BUILD_END_EVENT, input, null);
        this.emit(IconBundler.BUNDLE_END_EVENT, this.result);
     ...
     */
    async write() {
        this.emit(IconBundler.WRITE_START_EVENT);
        const result = await Promise.all(
            this.result.map(async ({ file, buffer }) => {
                await buffer.writeAsync(file.path);
                this.emit(IconBundler.WRITE_PROGRESS_EVENT, file);
                return file;
            })
        );
        this.emit(IconBundler.WRITE_END_EVENT);
        await super.write();
        return result;
    }
}

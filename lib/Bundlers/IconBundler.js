...
class IconBundler extends Bundler {
    ...
    async setup(options = {}) {
        ...
        if (typeof input === 'string') {
            options.input = new NavigatorFile(input);
        }
        options.output = output.map((file) => {
            if (typeof file === 'string') {
                return new NavigatorFile(file, input.cwd);
            }
            return file;
        });
    ...
    async build(...invalidate) {
        ...
        this.emit(IconBundler.START_EVENT, input, null, invalidate);
        ...
        this.emit(IconBundler.END_EVENT);
    ...
    async write() {
        return await Promise.all(
            this.result.map(async ({ file, buffer }) => {
                ...
                this.emit(IconBundler.WRITE_EVENT, file);
                return file;
            })
        );
    }

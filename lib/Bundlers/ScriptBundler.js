...
class ScriptBundler extends Bundler {
    ...
    async setup(options = {}) {
        ...
        let { input, output, map, code, root, format, coverage, jsx, production, bundle, checkDependencies = true } = options;
            ...
            plugins: [
                {
                    resolveId(importee, importer) {
                        ...
                        if (!bundle && !iife && !path.isAbsolute(importee) && !importee.match(/\.\.?\//)) {
                            if (importee.indexOf('@babel/runtime') !== -1 ||
                                importee.indexOf('core-js') !== -1) {
                                return null;
                            }
                        ...
                        if (checkDependencies && dependencies.indexOf(moduleName) === -1) {
    ...
    async build(invalidate) {
        ...
        let { input, output, root, code, map, production, format, plugins, vendors, chunks = {}, sw, preserveEntrySignatures } = this.options;
        ...
        try {
            ...
            let config = {
                ...
                plugins,
                inlineDynamicImports: inlineDynamicImports || !!sw,
                preserveSymlinks: true,
                preserveEntrySignatures: typeof preserveEntrySignatures === 'boolean' ? preserveEntrySignatures : true,
                onwarn: (warning) => {

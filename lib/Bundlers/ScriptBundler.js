...
class ScriptBundler extends Bundler {
    ...
    async setup(options = {}) {
        ...
        let { input, output, code, root, format, coverage, jsx, production, bundle } = options;
        if (!input && !code) {
            ...
            plugins: [
                {
                    resolveId(importee, importer) {
                        ...
                        if (!bundle && !legacy && !path.isAbsolute(importee) && !importee.match(/\.\.?\//)) {
                    ...
                    load(id) {
                        ...
                        try {
                            if (id && legacy && resolve.isCore(id)) {
                                // core nodejs modules
    ...
    async build(...invalidate) {
        ...
        const { input, output, root, code, map, format, cache, plugins, vendors } = this.options;
        let legacy = (format === 'umd' || format === 'iife');

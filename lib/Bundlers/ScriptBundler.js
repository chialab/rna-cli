...
class ScriptBundler extends Bundler {
    ...
    async build(...invalidate) {
        ...
        try {
            if (progress) {
                logger.play(`generating script${invalidate.length ? ' (this will be fast)...' : '...'}`, input ? input.localPath : '');
            }
            ...
            if (write && !code) {
                let { output } = await this.result.write(config.output);
                this.result.code = output[0].code;
            } else {
                let { output } = await this.result.generate(config.output);
                this.result.code = output[0].code;
            }

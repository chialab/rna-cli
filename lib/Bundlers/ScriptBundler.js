...
const { NavigatorFile, NavigatorDirectory } = require('../Navigator');
...
class ScriptBundler extends Bundler {
    ...
    async setup(options = {}) {
        ...
        if (typeof output === 'string') {
            if (path.extname(options.output)) {
                ...
                options.output = output = new NavigatorDirectory(output);
            }
        }
        if (typeof root === 'string') {
            options.root = root = new NavigatorDirectory(root);
        } else if (!root) {
    ...
    async build(...invalidate) {
        ...
        this.emit(ScriptBundler.START_EVENT, input, code, invalidate);
        ...
        try {
            ...
            if (this.linter && (this.linter.hasErrors() || this.linter.hasWarnings())) {
                this.emit(ScriptBundler.LINT_EVENT, this.linter.report());
            }

            this.emit(ScriptBundler.END_EVENT);
    ...
    async write() {
        ...
        if (output) {
            ...
            res.output
                ...
                .forEach((out) => {
                    if (output.extname) {
                        this.emit(ScriptBundler.WRITE_EVENT, output.parent.file(out.fileName));
                    } else {
                        this.emit(ScriptBundler.WRITE_EVENT, output.file(out.fileName));
                    }
                });
        }
        return output;

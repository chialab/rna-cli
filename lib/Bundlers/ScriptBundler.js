...
class ScriptBundler extends Bundler {
    ...
    async setup(options = {}) {
        ...
        let plugins = [
            ...
            url({
                exclude: [
                    /\.(mjs|js|jsx|ts|tsx|po|json|txt|md|html|htm|css|scss|sass)$/,
                ],
                include: [],
            }),
            html({
    ...
    async build(...invalidate) {
        ...
        try {
            ...
            let config = {
                ...
                cache: Object.assign(
                    cache.toConfig(this.options.get('format')),
                    {
                        plugins: this._pluginsCache || {},
                    },
                ),
            ...
            this._pluginsCache = this.result.cache.plugins;
    ...
    async write() {
        ...
        let oldFiles = this.result.outputFiles || [];
        let outputResult = await this.result.write(this.config.output);

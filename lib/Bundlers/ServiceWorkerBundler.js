...
class ServiceWorkerBundler extends ScriptBundler {
        ...
        try {
            ...
            if (output && output.exists()) {
                ...
                if (content.match(/\.(precache|precacheAndRoute)\s*\(\s*\[([^\]]*)\]\)/i)) {
                    let tmpInput = new File(path.join(require('os').tmpdir(), output.name));
                    content = content.replace(/\.(precache|precacheAndRoute)\s*\(\s*\[([^\]]*)\]\)/gi, '.$1([])');
                    tmpInput.write(content);
    ...
    async write() {
        ...
        await Bundler.prototype.write.call(this);
        return output;
    }
}
